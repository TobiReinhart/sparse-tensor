...
module Math.Tensor.Examples.Gravity (
    ...
    randAreaDerivative1 = do gen <- newTFGen 
                             ...
                             let f a p = (Empty, singletonInd $ Ind20 a, Empty, Empty, Empty, singletonInd $ Ind3 p)
                             return $ fromListT6 $ zip inds randList
    ...
    randAreaDerivative2 = do gen <- newTFGen 
                             ...
                             let f a i = (Empty, singletonInd $ Ind20 a, Empty, singletonInd $ Ind9 i, Empty, Empty)
                             return $ fromListT6 $ zip inds randList
