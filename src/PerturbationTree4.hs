...
module PerturbationTree4 (
    ...
    data AnsatzForest a = Forest (M.Map a (AnsatzForest a))| FLeaf a | EmptyForest  deriving (Show, Eq)

    mapLeafs :: (a -> a) -> AnsatzForest a -> AnsatzForest a
    mapLeafs f EmptyForest = EmptyForest 
    mapLeafs f (FLeaf x) = FLeaf (f x) 
    mapLeafs f (Forest m) = Forest $ M.map (mapLeafs f) m 

    filterForest :: (AnsatzForest a -> Bool) -> AnsatzForest a -> AnsatzForest a
    filterForest f (Forest m) = Forest (M.filter f m) 
    filterForest f t = t 
    ...
    mkForestFromAscList (x:xs) = Forest $ M.fromAscList [(x, mkForestFromAscList xs)]
    ...
    addToForest [x] (FLeaf y)
                ...
                | otherwise = EmptyForest
                 where 
                    val = addVars x y
    addToForest (x:xs) (Forest m) = Forest $ M.filter (/= EmptyForest) $ M.alter (updateF (x:xs)) x m 

    updateF :: (Ord a) => [AnsatzNode a] -> Maybe (AnsatzForest (AnsatzNode a)) -> Maybe (AnsatzForest (AnsatzNode a))
    updateF (x:xs) Nothing = Just $ mkForestFromAscList xs 
    updateF (x:xs) (Just mForest) = Just $ addToForest xs mForest  
