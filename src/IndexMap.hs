...
module IndexMap (
   ...
   data Ind (n::Nat) a = UnsafemkInd (I.IntMap a) deriving (Show,Ord,Eq)

   --try IntMap this time, int yields index position

   --we need a smart (value-) constructor (fromList ?)

   mkInd :: forall n a. KnownNat n => (I.IntMap a) -> Ind n a
   mkInd l 
           | I.size l == nat = UnsafemkInd l
   ...
   getRangeListL :: Enum a => Int -> Int -> [[a]]
   getRangeListL i r
           | i == 0 = [[]]
           | i == 1 = [ [a] | a <- [ toEnum 0.. toEnum r]]
           | otherwise = [ a : b | a <- [ toEnum 0.. toEnum r], b <- getRangeListL (i-1) r]  

   getRangeList :: Enum a => Int -> Int -> [ I.IntMap a ]
   getRangeList i r = map (I.fromList) l
                where 
                    l = map (\x -> zip [0..] x) $ getRangeListL i r 

   --start with the symmetrizers 

   swapPosF :: (Int,Int) -> Int -> Int 
   swapPosF (i,j) x 
            | x == i = j 
            | x == j = i 
            | otherwise = x 

   swapPosSeq :: (Int,Int) -> (I.IntMap a) -> (I.IntMap a)
   swapPosSeq (i,j) m = I.mapKeys (swapPosF (i,j)) m 


   swapBlockPosSeq :: ([Int],[Int]) -> (I.IntMap a) -> (I.IntMap a)
   ...
   update1CycleF :: ([Int],[Int]) -> Int -> Int
   update1CycleF (i,j) x = fromMaybe x yMaybe 
            where
                exchangeMap = I.fromList $ zip i j 
                yMaybe = I.lookup x exchangeMap 


   update1Cycle :: ([Int],[Int]) -> (I.IntMap a) -> (I.IntMap a)
   update1Cycle (i,j) s = I.mapKeys (update1CycleF (i,j)) s  

   cyclicSwapSeq :: [Int] -> (I.IntMap a) -> [(I.IntMap a)]
   ...
   indexList a1 b1 c1 d1 e1 f1 g1 h1 = (a2,b2,c2,d2,e2,f2,g2,h2)
           where 
               a2 = mkInd $ I.fromList $ zip [0..] (map toEnum a1)
               b2 = mkInd $ I.fromList $ zip [0..] (map toEnum b1)
               c2 = mkInd $ I.fromList $ zip [0..] (map toEnum c1)
               d2 = mkInd $ I.fromList $ zip [0..] (map toEnum d1)
               e2 = mkInd $ I.fromList $ zip [0..] (map toEnum e1)
               f2 = mkInd $ I.fromList $ zip [0..] (map toEnum f1)
               g2 = mkInd $ I.fromList $ zip [0..] (map toEnum g1)
               h2 = mkInd $ I.fromList $ zip [0..] (map toEnum h1)
   ...
   isContractionInd (i,k) (UnsafemkInd s1) (UnsafemkInd s2) = val1 == val2
               where 
                   val1 = fromEnum $ fromJust $ I.lookup i s1
                   val2 = fromEnum $ fromJust $ I.lookup k s2
   ...
   checkInd (UnsafemkInd s) i val = (fromJust $ I.lookup i s) == val  
